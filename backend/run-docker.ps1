...
$AppProjectName = "libremeet-backend"
$AppContainerName = "libremeet-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "libremeet-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server libremeet-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server libremeet-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "libremeet-backend"
        "backend" = "libremeet-backend"
        "meeting" = "libremeet-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "libremeet-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, libremeet-backend"
        ...
        $logArgs += @("whisper-server", "libremeet-backend")
    }
