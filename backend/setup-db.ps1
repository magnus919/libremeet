...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.libremeet\meeting_minutes.db",
        "$env:USERPROFILE\Documents\libremeet\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/libremeet-backend/*/backend/meeting_minutes.db"
    }
