...
Write-Host "====================================="
Write-Host "LibreMeet Backend Startup"
Write-Host "====================================="
...
if (-not (Test-Path "whisper-server-package\whisper-server.exe")) {
    ...
    try {
        ...
        $apiUrl = "https://api.github.com/repos/libremeet/libremeet/releases/latest"
        ...
        $downloadUrl = "https://github.com/libremeet/libremeet/releases/download/$tagName/whisper-server.exe"
        ...
        try {
            $allReleasesUrl = "https://api.github.com/repos/libremeet/libremeet/releases"
            $headers = @{"User-Agent" = "PowerShell-Script"}
            ...
            if ($releases.Count -gt 0) {
                ...
                $altDownloadUrl = "https://github.com/libremeet/libremeet/releases/download/$latestTag/whisper-server.exe"
            ...
            Write-Host "https://github.com/libremeet/libremeet/releases"
...
Write-Host "====================================="
Write-Host "Starting LibreMeet Backend"
Write-Host "====================================="
...
# Check if libremeet-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for libremeet-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\libremeet-frontend\libremeet-frontend.exe",
    "$env:LOCALAPPDATA\Programs\libremeet\libremeet-frontend.exe",
    "$env:ProgramFiles\libremeet-frontend\libremeet-frontend.exe",
    "${env:ProgramFiles(x86)}\libremeet-frontend\libremeet-frontend.exe",
    "$env:APPDATA\libremeet-frontend\libremeet-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("libremeet-frontend.exe", "libremeet.exe")
...
if ($frontendInstalled) {
    Write-Host "LibreMeet frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the LibreMeet frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching LibreMeet frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "LibreMeet frontend launched successfully."
        }
    }
} else {
    Write-Host "LibreMeet frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the LibreMeet frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if libremeet is now installed and launch it
