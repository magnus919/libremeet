...
show_log_exit_options() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "whisper-server\|libremeet-backend"; then
...
find_existing_databases() {
    local found_dbs=()
    local default_db_path="/opt/homebrew/Cellar/libremeet-backend/0.0.4/backend/meeting_minutes.db"
    ...
    local common_paths=(
        "/opt/homebrew/Cellar/libremeet-backend/*/backend/meeting_minutes.db"
        "$HOME/.libremeet/meeting_minutes.db"
        "$HOME/Documents/libremeet/meeting_minutes.db"
...
start_server() {
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "libremeet-backend:"; then
...
show_status() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "libremeet-backend"; then
        app_running=true
        local app_port=$(docker port libremeet-backend 5167/tcp 2>/dev/null | cut -d: -f2)
        if [ -n "$app_port" ]; then
            log_info "Meeting App: http://localhost:$app_port"
            # Test connectivity
...
open_shell() {
    ...
    case "$service" in
        "whisper")
            ...
            ;;
        "app"|"backend")
            container_name="libremeet-backend"
            ;;
        *)
